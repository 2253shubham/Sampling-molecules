...
def extract_energy(linecount, mol_size, readf, start):
    ...
    for i in range(start-1, linecount, mol_size+start):
        whole_line = readf[i]
        line_array = [float(item) if item[0] != "*" else float("Nan")
...
def merge_files(file1, file2, start, m1, out_file):
    ...
    for i in range(l1-1):
        print("9", end=" \n", file=fileout)
        [print(str(r1[j]), end=" ", file=fileout) for j in range(i*(m1+start)+1, (i+1)*(m1+start))]
        [print(str(r2[j]), end=" ", file=fileout) for j in range(i*(5+start)+start, (i+1)*(5+start))]
    fileout.close()
...
if __name__ == "__main__":
    ...
    if (args.inp_file_2=='system.xyz'):
        ...
        write_sorted_configurations_1(args.out_file, index_sorted,
                                    readf, int(args.mol_size), start)
        write_sorted_configurations_2("sorted_system.xyz", index_sorted,
                                    sys_readf, sys_size, start)
        merge_files(args.out_file, "sorted_system.xyz", start, int(args.mol_size), "final_struct.xyz")
    else:
        write_sorted_configurations_1(args.out_file, index_sorted,
                                    readf, int(args.mol_size), start)
