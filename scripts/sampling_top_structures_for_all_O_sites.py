...
def center_of_mass_data_and_hydrogen_coor_and_energies(readf, linecount, mol_size, start, elements, counter, gh, rxn): # modified to C-O bond distance for TS
    c = 1
    com_coor_data = []
    energies = []
    hydrogen_coor = []
    ...
    for i in range(0, linecount-1, mol_size+start):
        coordinates = np.array([[float(item) for item in whole_line.split()[
                               1:4]] for whole_line in readf[i+start:c*(mol_size+start)]])
        energies.append(np.float(readf[i+start-1].split()[0]))
        #com_coor = com(coordinates, elements) # without surface atoms 
        if(counter==1):
            com_coor = coordinates[len(coordinates)-3-1] # if C-O distance is considered
            ...
            com_coor = com(coordinates[:len(coordinates)-3], elements) # without surface atoms
            com_coor_data.append(com_coor)
            hydrogen_coor.append(coordinates[len(coordinates)-3-3]) # actually carbon atom (change accordingly) (1 for C2C2, 2 for C2C3, 3 for C2mC3)
            n1.append(coordinates[len(coordinates)-3-4]) # actually carbon atom (change accordingly) (2 for C2C2, 3 for C2C3, 4 for C2mC3)
        elif(counter>1 and gh==0 and rxn=="ht"):
            com_coor = com(coordinates[:len(coordinates)-5], elements) # without surface atoms (only the adsorbed molecule)
            com_coor_data.append(com_coor)
            hydrogen_coor.append(coordinates[len(coordinates)-3-1]) # fixed carbon atom for all systems
        ...
        #hydrogen_coor.append(coordinates[4])
        c+=1
    return np.array(com_coor_data).reshape(-1,3), np.array(energies).reshape(-1,1), np.array(hydrogen_coor).reshape(-1,3), np.array(n1).reshape(-1,3)
...
def closest(top_O_sites, ns, com_data, wcom_data, O_coor, wO_coor, si_coor, lattice, cut_off, counter_array, com_counter_array, filename1, filename2, n1, ref_angle, ref_dis, h_coor, gh, rxn, zeo):
    min_dis = 100
    prox = []
    min_dis_index = 0
    f = 100
    for i in range(len(O_coor)):
        ...
        diff = np.linalg.norm(pbc_diff(com_data/lattice.T,O_coor[i]/lattice.T)*lattice.T, axis=1)
        diff_rs = np.linalg.norm(pbc_diff(h_coor/lattice.T,O_coor[i]/lattice.T)*lattice.T, axis=1)
        #print(diff_rs)
        ...
        for j in range(len(O_coor[i])):
            cosine_angle = np.dot(d1, d2[j]) / (np.linalg.norm(d1) * np.linalg.norm(d2[j]))
...
#            f = i
    ...
    if (min_dis != 100):
        if (counter_array[f]<ns):
            if (zeo == "TON"):
                cm = clone(filename2)
                [cm.OBMol.DeleteAtom(cm.atoms[i].OBAtom) for i in range(len(cm.atoms))[::-1]]
                a1 = openbabel.OBAtom()
            ...
            if (np.min(prox)>cut_off):
                ...
                diff_sio = np.linalg.norm(si_coor-np.array(O_coor[f][min_dis_index]), axis=1)
                ...
                if (abs(min_dsio1-min_dsio2)<0.5):
                    ...
                    if (min1>min2):
                        flag = 2
                        y = extend_single_cell_si_atoms(dim2, np.array(x), lattice, flag)
                    else:
                        flag = 1
                        y = extend_single_cell_si_atoms(dim1, np.array(x), lattice, flag)
                    diff_sio = np.linalg.norm(y-np.array(O_coor[f][min_dis_index]), axis=1)
                    min_dsio2 = np.min(diff_sio)
                    sic2 = np.argmin(diff_sio)
                    si2 = y[sic2]
                    v2 = omid-si2
                ...
                d1 = np.linalg.norm(pbc_diff(h_loc1/lattice.T,com_data/lattice.T)*lattice.T)
                h_loc2 = omid+(v3/np.linalg.norm(v3))
                d2 = np.linalg.norm(pbc_diff(h_loc2/lattice.T,com_data/lattice.T)*lattice.T)
                if (d1>d2):
                    h_loc = h_loc2
                else:
                    h_loc = h_loc1        
...
def com(coordinates, elements):
    ...
    mwc = np.sum([coordinates[i]*mydict[elements[i]]
                  for i in range(len(coordinates))], axis=0)
    return mwc/totmass
 

def create_cif_file(lattice, coor):
    my_lattice = Lattice(
        [[lattice[0], 0, 0], [0, lattice[1], 0], [0, 0, lattice[2]]])
    structure = Structure(my_lattice, ["C", "C", "C", "C"], [
                          coor[0], coor[1], coor[2], coor[3]], coords_are_cartesian=True)
    j = CifWriter(structure)
    j.write_file("temp_cif.cif")


def create_cif_file_of_samp_molecule(filename, lattice, final_coor, elements, str_num, name, cut_off, gh, oc):
    ...
    j = CifWriter(structure)
    directory = "distinct_sites/"+str(cut_off)+"/"+str(name)+"/" # change this +"C-O-dis"
    if not os.path.exists(directory):
        os.makedirs(directory)
...
def create_final_pdb_with_zeolite(tr_coor, sial, bu_elements, lattice, filename, name, cut_off, str_num, energy):
    molecule = Molecule.from_file(filename)
    ...
    my_lattice = Lattice(
        [[lattice[0], 0, 0], [0, lattice[1], 0], [0, 0, lattice[2]]])
    structure = Structure(my_lattice, all_elements, 
                          coord_nf, coords_are_cartesian=True)
    j = CifWriter(structure)
    j.write_file("temp_final_cif.cif")
    mol = pybel.readfile("cif", "temp_final_cif.cif").__next__()
    ...
    print(directory)
    print("*****************************")
    mol.write("pdb", directory+"structure_num_"+str(int(str_num))+"_of_butane.pdb" , overwrite=True)
...
def energy_parser(energy, top_O_sites, com_data, n1, O_coor, lattice, ref_angle, ref_dis, h_coor, rxn): # for transition states
    min_dis = 100
    prox = []
    min_dis_index = 0
    f = 100
...
def exec_code(freqIntersection, freqSinusoidal, freqStraight, readf, mol_size, num_of_struct, start, elements, energy_array, reference_i_com, reference_z_com, reference_s_com):
    c = 1
    fx = 0
    fy = 0
    fz = 0
    ...
    for i in range(0, num_of_struct*(mol_size+start)-1, mol_size+start):
        ...
        if (freqIntersection == 1 and fx == 0):
            uc_coord = [unit_cell_coordinates(
                coordinates[i]) for i in range(len(coordinates))]
            write_top_configurations(
                readf, mol_size, "top_freq_intersection.xyz", int(i/(mol_size+start)), coord=uc_coord)
            fx = 1
        if (freqSinusoidal == 1 and fy == 0):
            uc_coord = [unit_cell_coordinates(
                coordinates[i]) for i in range(len(coordinates))]
            write_top_configurations(
                readf, mol_size, "top_freq_sinusoidal.xyz", int(i/(mol_size+start)), coord=uc_coord)
            fy = 1
        if (freqStraight == 1 and fz == 0):
            uc_coord = [unit_cell_coordinates(
                coordinates[i]) for i in range(len(coordinates))]
            write_top_configurations(
                readf, mol_size, "top_freq_straight.xyz", int(i/(mol_size+start)), coord=uc_coord)
            fz = 1
        if (freqIntersection-a > 0):
            print(energy_array[int(i/(mol_size+start))], file=out3)
            max_dis = max_dis_between_two_structures(reference_i_com, np.array(unit_cell_coordinates(com_coor)))
            print(max_dis)
            if (max_dis<5 and di<=20):
                ...
                uc_coord = [unit_cell_coordinates(coordinates[i]) for i in range(len(coordinates))]
                write_top_configurations(readf, mol_size, fname, int(i/(mol_size+start)), coord=uc_coord)
                di+=1
        if (freqSinusoidal-b > 0):
            print(energy_array[int(i/(mol_size+start))], file=out2)
            max_dis = max_dis_between_two_structures(reference_z_com, np.array(unit_cell_coordinates(com_coor)))
            print(max_dis)
        ...
        if (freqStraight-d > 0):
            ...
            max_dis = max_dis_between_two_structures(reference_s_com, np.array(unit_cell_coordinates(com_coor)))
            print(max_dis)
...
def grow_hydrogen_to_reactant_state_bs(filename, lattice, final_coor, elements, str_num, name, cut_off, gh, oc, sial):
    my_lattice = Lattice([[lattice[0], 0, 0], [0, lattice[1], 0], [0, 0, lattice[2]]])
    my_lattice_2 = Lattice([[lattice[0]*2, 0, 0], [0, lattice[1]*2, 0], [0, 0, lattice[2]*2]])
    #coord = retransform(final_coor, lattice)
    molecule = Molecule.from_file(filename)
    coord_f = molecule.cart_coords
    #coord = retransform(final_coor, lattice)
...
#        b2.SetBondOrder(1)
    ...
    print("all_coord")
    print(coord)
    diff = np.linalg.norm(pbc_diff(oc/lattice.T,coord/lattice.T)*lattice.T, axis=1)
    print("diff")
    print(diff)
    min_index = np.argmin(np.abs(diff))
    print("oc")
...
def grow_hydrogen_to_reactant_state_ht(filename, lattice, final_coor, elements, str_num, name, cut_off, gh, oc, sial):
    my_lattice = Lattice([[lattice[0], 0, 0], [0, lattice[1], 0], [0, 0, lattice[2]]])
    my_lattice_2 = Lattice([[lattice[0]*2, 0, 0], [0, lattice[1]*2, 0], [0, 0, lattice[2]*2]])
    #coord = retransform(final_coor, lattice)
    molecule = Molecule.from_file(filename)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Sample lower energy structures')
    parser.add_argument('-i1', '--inp1_file', action='store',
                        default='inp1.cif')
    parser.add_argument('-i2', '--inp2_file', action='store',
                        default='sorted_struct.txt')
    parser.add_argument('-i3', '--inp3_file', action='store',
                        default='inp2.cif')
    ...
    parser.add_argument('-dis', '--ref_dis', action='store', default='2') # change for bs (1.52 for C2C2, 1.56 for C2C3, 1.66 for C2mC3), for ht (fixed 1.52)
    parser.add_argument('-gh', '--grow_h', action='store', default='1') # 0 to grow hydrogens, 1 for not to grow hydrogens
    parser.add_argument('-rxn', '--rxn_type', action='store', default='bs') # bs = beta scission, ht = hydride transfer   
...
def retransform(array, lattice):
    for j in range(3):
        while (any(array[:,j]<0) or any(array[:,j]>lattice[j])):
            ...
            array[:,j][array[:,j]>lattice[j]] = array[:,j][array[:,j]>lattice[j]]-lattice[j]
    return array
...
def screen_atoms(coords):
    ...
    within_range = np.any((coords[:, 0] >= x_range[0]) &
                        (coords[:, 0] <= x_range[1]) &
                        (coords[:, 1] >= y_range[0]) &
                        (coords[:, 1] <= y_range[1]) &
                        (coords[:, 2] >= z_range[0]) &
...
def transformations(x,y,z,array,lattice):
    ...
    array[1] = np.array([(-x+0.5)*lattice[0], -y*lattice[1], (z+0.5)*lattice[2]])
    array[2] = np.array([-x*lattice[0], (y+0.5)*lattice[1], -z*lattice[2]])
    array[3] = np.array([(x+0.5)*lattice[0], (-y+0.5)*lattice[1], (-z+0.5)*lattice[2]])
    array[4] = np.array([-x*lattice[0], -y*lattice[1], -z*lattice[2]])
    array[5] = np.array([(x+0,5)*lattice[0], y*lattice[1], (-z+0.5)*lattice[2]])
    array[6] = np.array([x*lattice[0], (-y+0.5)*lattice[1], z*lattice[2]])
    array[7] = np.array([(-x+0.5)*lattice[0], (y+0.5)*lattice[1], (z+0.5)*lattice[2]])
    array[:,0][array[:,0]<0] = array[:,0][array[:,0]<0] + lattice[0]
    array[:,0][array[:,0]>lattice[0]] = array[:,0][array[:,0]>lattice[0]] - lattice[0]
    array[:,1][array[:,1]<0] = array[:,1][array[:,1]<0] + lattice[1]
    array[:,1][array[:,1]>lattice[1]] = array[:,1][array[:,1]>lattice[1]] - lattice[1]
    array[:,2][array[:,2]<0] = array[:,2][array[:,2]<0] + lattice[2]
    array[:,2][array[:,2]>lattice[2]] = array[:,2][array[:,2]>lattice[2]] - lattice[2]
    return array


def unit_cell_coordinates(coord, lattice):
    mod_coord = [[coord1[i]*float(lattice[i]) for i in range(len(coord1))] for coord1 in coord]
    coord_wrt_lat = [[(mod_coord1[i] < 0) and float(lattice[i]) +
                     mod_coord1[i] or mod_coord1[i] for i in range(len(mod_coord1))] for mod_coord1 in mod_coord]
    return coord_wrt_lat


def within_half(coord, lattice):
    mod_coord = [[(mod_coord1[i] > float(lattice[i])/2) and mod_coord1[i] - float(lattice[i]) or mod_coord1[i] for i in range(len(mod_coord1))] for mod_coord1 in coord]
...
if __name__ == "__main__":
    ...
    linecount = len(readf)
    lattice = make_lattice(float(args.lattice_x), float(args.lattice_y), float(args.lattice_z))
    ...
    com_data, energies, hydrogen_coor_data, n1 = center_of_mass_data_and_hydrogen_coor_and_energies(readf, linecount, int(args.mol_size), start, elements, int(args.num_of_struct), int(args.grow_h), args.rxn_type)
    ...
    O_coor = np.array([[coor[a[j]] for j in range(len(a))] for a in pos[0:len(un_atom_names)]])
    wO_coor = np.array([[wcoor[a[j]] for j in range(len(a))] for a in pos[0:len(un_atom_names)]])

    if (int(args.num_of_struct)==1):
        for i in range(len(com_data)):
            energy_parser(energies[i], un_atom_names, com_data[i], n1[i], O_coor, lattice, float(args.ref_angle), float(args.ref_dis), hydrogen_coor_data[i], args.rxn_type)


    for i in range(len(com_data)):
        if (int(args.num_of_struct)==1):
            O_name_index, closest_O_index, h_loc, sial, oc = closest(un_atom_names, int(args.num_of_struct), com_data[i], wcom_data[i], O_coor, wO_coor, si_coor, lattice, float(args.cut_off), counter_array, com_counter_array, args.inp1_file, ars.inp3_file, n1[i], float(args.ref_angle), float(args.ref_dis), hydrogen_coor_data[i], int(args.grow_h), args.rxn_type, args.zeo_name)
            if (O_name_index!=-1):
                final_coor = np.array([[float(item) for item in whole_line.split()[1:4]] for whole_line in readf[(i)*(int(args.mol_size)+start)+start:(i+1)*(int(args.mol_size)+start)]])
                create_cif_file_of_samp_molecule("osite_info.xyz", lattice, final_coor[:len(final_coor)-3], elements[:len(elements)-3], counter_array[O_name_index], un_atom_names[O_name_index], float(args.cut_off), int(args.grow_h), oc)
        elif (int(args.num_of_struct)>1 and int(args.grow_h)==0 and (args.rxn_type=="bs")):
            O_name_index, closest_O_index, h_loc, sial, oc = closest(un_atom_names, int(args.num_of_struct), com_data[i], wcom_data[i], O_coor, wO_coor, si_coor, lattice, float(args.cut_off), counter_array, com_counter_array, args.inp1_file, args.inp3_file, n1[i], float(args.ref_angle), float(args.ref_dis), hydrogen_coor_data[i], int(args.grow_h), args.rxn_type, args.zeo_name)
            if (O_name_index!=-1):
                final_coor = np.array([[float(item) for item in whole_line.split()[1:4]] for whole_line in readf[(i)*(int(args.mol_size)+start)+start:(i+1)*(int(args.mol_size)+start)]])
                grow_hydrogen_to_reactant_state_bs("osite_info.xyz", lattice, final_coor[:len(final_coor)-3], elements[:len(elements)-3], counter_array[O_name_index], un_atom_names[O_name_index], float(args.cut_off), int(args.grow_h), oc, sial)            
        elif (int(args.num_of_struct)>1 and int(args.grow_h)==0 and (args.rxn_type=="ht")):
            O_name_index, closest_O_index, h_loc, sial, oc = closest(un_atom_names, int(args.num_of_struct), com_data[i], wcom_data[i], O_coor, wO_coor, si_coor, lattice, float(args.cut_off), counter_array, com_counter_array, args.inp1_file, args.inp3_file, n1[i], float(args.ref_angle), float(args.ref_dis), hydrogen_coor_data[i], int(args.grow_h), args.rxn_type, args.zeo_name)
            if (O_name_index!=-1):
                final_coor = np.array([[float(item) for item in whole_line.split()[1:4]] for whole_line in readf[(i)*(int(args.mol_size)+start)+start:(i+1)*(int(args.mol_size)+start)]])
                grow_hydrogen_to_reactant_state_ht("osite_info.xyz", lattice, final_coor[:len(final_coor)-3], elements[:len(elements)-3], counter_array[O_name_index], un_atom_names[O_name_index], float(args.cut_off), int(args.grow_h), oc, sial)
        else:
            O_name_index, closest_O_index, h_loc, sial, oc = closest(un_atom_names, int(args.num_of_struct), com_data[i], wcom_data[i], O_coor, wO_coor, si_coor, lattice, float(args.cut_off), counter_array, com_counter_array, args.inp1_file, args.inp3_file, n1[i], float(args.ref_angle), float(args.ref_dis), hydrogen_coor_data[i], int(args.grow_h), args.rxn_type, args.zeo_name)
            if (O_name_index!=-1):
                final_coor = np.array([[float(item) for item in whole_line.split()[1:4]] for whole_line in readf[(i)*(int(args.mol_size)+start)+start:(i+1)*(int(args.mol_size)+start)]])
                ...
                create_final_pdb_with_zeolite(
                    mod_tr_coor, sial, elements, lattice, "osite_info.xyz", un_atom_names[O_name_index], float(args.cut_off), counter_array[O_name_index], energies[i])
